...
void Application::DrawGUI(void)
{
	...
	//About
	{
		...
		ImGui::Begin(sAbout.c_str(), (bool*)0, window_flags);
		{
			...
			ImGui::Text("Showing Octant: %d", m_uOctantID);
			ImGui::Text(" Octant levels: %d", m_uOctantLevels);
			ImGui::Text("FrameRate: %.2f [FPS] -> %.3f [ms/frame]",
				ImGui::GetIO().Framerate, 1000.0f / ImGui::GetIO().Framerate);
			if(m_uOctantLevels == 0)
				ImGui::TextColored(v4Color, "Brute Force Check!");
			else
			{
				v4Color = ImColor(0, 255, 0);
				ImGui::TextColored(v4Color, "Optimization on");
			}
		}
		ImGui::End();
	}
